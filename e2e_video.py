"""End-to-end video generation test."""

import asyncio
import aiohttp
from pathlib import Path
from datetime import datetime

from vibe_aigc.vibe_backend import VibeBackend, GenerationRequest
from vibe_aigc.discovery import Capability

OUTPUT_DIR = Path("./generated_outputs")
OUTPUT_DIR.mkdir(exist_ok=True)

async def main():
    print("=" * 60)
    print("VIBE-AIGC -- TEXT TO VIDEO E2E")
    print("=" * 60)
    print()
    
    backend = VibeBackend(
        comfyui_url="http://192.168.1.143:8188",
        enable_vlm=True,
        max_attempts=1,  # Video takes longer, single attempt
        quality_threshold=6.0
    )
    
    print("Initializing...")
    await backend.initialize()
    print()
    
    prompt = "cyberpunk samurai walking through neon rain, cinematic slow motion"
    
    print(f"Generating video: {prompt}")
    print()
    
    result = await backend.generate(GenerationRequest(
        prompt=prompt,
        capability=Capability.TEXT_TO_VIDEO,
        width=512,
        height=320,
        frames=48,  # ~2 seconds at 24fps
        steps=20,
        cfg=7.0
    ))
    
    print()
    print("=" * 60)
    print("RESULT")
    print("=" * 60)
    print(f"Success: {result.success}")
    print(f"Attempts: {result.attempts}")
    print(f"Quality Score: {result.quality_score}/10")
    print()
    
    if result.output_url:
        print(f"Remote URL: {result.output_url}")
        
        # Download
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ext = '.webp' if 'webp' in result.output_url else '.mp4'
        local_path = OUTPUT_DIR / f"e2e_video_{timestamp}{ext}"
        
        async with aiohttp.ClientSession() as session:
            async with session.get(result.output_url) as resp:
                if resp.status == 200:
                    content = await resp.read()
                    local_path.write_bytes(content)
                    print(f"Downloaded to: {local_path.absolute()}")
                    return result, local_path
    
    if result.feedback:
        print()
        print("VLM FEEDBACK:")
        print(f"  {result.feedback}")
    
    if result.strengths:
        print()
        print("STRENGTHS:")
        for s in result.strengths:
            print(f"  + {s}")
    
    if result.weaknesses:
        print()
        print("WEAKNESSES:")
        for w in result.weaknesses:
            print(f"  - {w}")
    
    if result.error:
        print(f"Error: {result.error}")
    
    print()
    return result, None

if __name__ == "__main__":
    result, path = asyncio.run(main())
    if path:
        print(f"\nVideo saved: {path}")
